    ...
    function Updater()  
        ...
        repeat task.wait()
            ...
            if plr.Character ~= nil and plr.Character:FindFirstChild("Humanoid") ~= nil and plr.Character:FindFirstChild("HumanoidRootPart") ~= nil and plr.Character.Humanoid.Health > 0 and plr.Character:FindFirstChild("Head") ~= nil then
                ...
                until game.Players:FindFirstChild(plr.Name) == nil or not game.Players:FindFirstChild(plr.Name).Character
                or not game.Players:FindFirstChild(plr.Name).Character:FindFirstChild('Humanoid') or game.Players:FindFirstChild(plr.Name).Character:FindFirstChild('Humanoid').Health <= 0
...
task.spawn(function()
    pcall(function() repeat task.wait(.01)
        pcall(function()
        if LocalPlayer.Character ~= nil and LocalPlayer.Character:FindFirstChild("Humanoid") ~= nil and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then 
                        ...
                        if OnScreen then
                            if v.LimitDistance then 
                                if distance > v.LimitDistance or not v.Enabled then 
                                        Library:Visibility(false, v['Item'])
                                    return
                                end
                            end
                            if v.Enabled then 
                                y.Visible = true
                                y.Color = v.Color
                                y.Position = Vector2.new(InsPos.X, InsPos.Y)
                                if v.ShowDistance then 
                                    y.Text = v['Name']..' ['..tostring(math.round(distance))..'m'..']'
                                    else
                                    y.Text = v['Name']
                                 end
                                 else
